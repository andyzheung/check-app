...
import com.pensun.checkapp.util.PasswordUtil;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public void updateUserPermissions(UserPermissionUpdateDTO updateDTO) {
        ...
        // 删除原有权限
        userPermissionMapper.deleteByUserId(updateDTO.getUserId());

        // 添加新权限
        if (updateDTO.getPermissions() != null && !updateDTO.getPermissions().isEmpty()) {
            List<UserPermission> permissions = updateDTO.getPermissions().stream()
                    .map(code -> {
                        UserPermission permission = new UserPermission();
                        permission.setUserId(updateDTO.getUserId());
