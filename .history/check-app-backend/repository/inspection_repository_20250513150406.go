...
func (r *InspectionRepository) GetRecordsByArea(areaID uint, startTime, endTime time.Time) ([]model.InspectionRecord, error) {
	var records []model.InspectionRecord
	err := r.db.Joins("JOIN inspection_points ON inspection_points.id = inspection_records.inspection_point_id").
		Where("inspection_points.area_id = ? AND inspection_records.created_at BETWEEN ? AND ?", areaID, startTime, endTime).
		Find(&records).Error
	return records, err
}
