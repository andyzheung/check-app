...
// GetInspectionPointByCode 根据编码获取巡检点
func (s *InspectionService) GetInspectionPointByCode(code string) (*model.InspectionPoint, error) {
	return s.inspectionRepo.GetPointByCode(code)
}

// CreateRecord 创建巡检记录
func (s *InspectionService) CreateRecord(record *model.InspectionRecord) error {
	return s.inspectionRepo.CreateRecord(record)
}

// GetRecordsByUser 获取用户的巡检记录
func (s *InspectionService) GetRecordsByUser(userID uint, page, pageSize int) ([]model.InspectionRecord, int64, error) {
	return s.inspectionRepo.GetRecordsByUser(userID, page, pageSize)
}

// GetRecordByID 根据ID获取巡检记录
func (s *InspectionService) GetRecordByID(id uint) (*model.InspectionRecord, error) {
	return s.inspectionRepo.GetRecordByID(id)
}

// UpdateRecord 更新巡检记录
func (s *InspectionService) UpdateRecord(record *model.InspectionRecord) error {
	return s.inspectionRepo.UpdateRecord(record)
}

// GetInspectionsByArea 获取指定区域的巡检记录
func (s *InspectionService) GetInspectionsByArea(areaID uint, startTime, endTime time.Time) ([]model.InspectionRecord, error) {
	return s.inspectionRepo.GetRecordsByArea(areaID, startTime, endTime)
}

// GetInspectionsByUser 获取指定用户的巡检记录
func (s *InspectionService) GetInspectionsByUser(userID uint, startTime, endTime time.Time) ([]model.InspectionRecord, error) {
	return s.inspectionRepo.GetRecordsByUserAndTime(userID, startTime, endTime)
}
