...
func (h *InspectionHandler) GetByArea(c *gin.Context) {
	...
	records, total, err := h.inspectionService.GetRecordsByArea(uint(areaID), page, pageSize)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *InspectionHandler) GetByUser(c *gin.Context) {
	...
	records, total, err := h.inspectionService.GetRecordsByUser(uint(userID), page, pageSize)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
